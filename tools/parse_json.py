...
LastEditTime: 2022-05-21 15:34:25
Description: 
...
def main():
    ...
    landmarks_path = os.path.dirname(img_list) + '_lms_withmask.txt'
    ...
    landmarks_path = os.path.dirname(img_list) + '_lms_filter.txt'
    lms_array_filter = np.array(lms_array_filter).reshape(-1, len(labels_set)*2)
    lms_array_filter_train = lms_array_withmask[withmask_train_idxs, :]
    lms_array_filter_test = lms_array_withmask[withmask_test_idxs, :]
